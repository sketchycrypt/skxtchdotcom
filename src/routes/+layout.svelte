...
// navbar icons (annoying)
import home from "$lib/assets/svgs/navbar/home.svg"
import projects from "$lib/assets/svgs/navbar/projects.svg"
import about from "$lib/assets/svgs/navbar/about.svg"
...
<svelte:head>
    ...
    <main class="font-instrumentsans font-bold relative text-white">
        <div class="relative z-10 flex flex-col min-h-screen">
            ...
            <nav class="bg-black  backdrop-blur-sm py-4 md:py-4 sticky top-0 z-20">
                <div class="container md:mx-auto px-4 sm:px-6">
                    <div class="hidden md:flex justify-center space-x-2 sm:space-x-4 md:space-x-6">
                        {#each navLinks as { href, text, icons }}
                        <a
                            href={href}
                            class="flex items-center gap-2 px-2 py-1 md:px-4 md:py-2 text-sm md:text-base rounded-lg transition-all duration-300
                            hover:bg-white/10 hover:scale-105
                            {$page.url.pathname === href ? 'bg-white/20 font-extrabold' : 'opacity-90'}"
                            >
                            <img src={icons} alt={text + ' icon'} class="w-4 h-4 md:w-5 md:h-5" />
                            {text}
                        </a>
                        {/each}
                    ...
                    <div class="md:hidden flex items-center justify-between">
                        ...
                        <div transition:fly={{y:-50, duration: 150}} class="absolute top-13 left-0 right-0 bg-black backdrop-blur-md py-4 px-6">
            ...
            <div
                ...
                class="container mx-auto p-6 flex-grow flex flex-col justify-center items-center text-center">
